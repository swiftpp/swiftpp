...
def create_argument_parser():
    ...
    option('--swift-compiler-version', store,
           type=argparse.SwiftPPVersionType(),
           metavar='MAJOR.MINOR',
    ...
    option('--swift-tools-max-parallel-lto-link-jobs', store_int,
           default=default_max_lto_link_job_counts['swiftpp'],
           metavar='COUNT',
           help='the maximum number of parallel link jobs to use when '
    ...
    option('--debug-swiftpp', store('swift_build_variant'),
           const='Debug',
...
debuggable Swift compiler:
  ...
  [~/src/s]$ ./swift/utils/build-script -R --debug-swiftpp
...
To use it, specify the '--preset=' argument:
  ...
  ./swift/utils/build-script --release --debug-swiftpp --debug-swift-stdlib \
     --test
  --build-subdir=ds --
  ...
