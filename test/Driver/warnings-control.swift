// RUN: not %target-ppswiftc_driver %s 2>&1 | %FileCheck -check-prefix=DEFAULT %s
// RUN: not %target-ppswiftc_driver -warnings-as-errors %s 2>&1 | %FileCheck -check-prefix=WERR %s
// RUN: not %target-ppswiftc_driver -suppress-warnings %s 2>&1 | %FileCheck -check-prefix=NOWARN %s

// RUN: not %target-ppswiftc_driver -suppress-warnings -warnings-as-errors %s 2>&1 | %FileCheck -check-prefix=FLAGS_CONFLICT %s
