// RUN: %swift_driver_plain --driver-mode=swiftppc -sdk "" -driver-print-actions %s 2>&1 | %FileCheck -check-prefix=CHECK-SWIFTC-%target-object-format %s
// RUN: %swift_driver_plain -sdk "" -driver-print-actions %s --driver-mode=swiftppc 2>&1 | %FileCheck -check-prefix=CHECK-SWIFT %s
