...
// RUN: %swiftppc_driver_plain -target %target-triple -module-cache-path %t -typecheck -Xfrontend -verify %s

// This test should be updated to match the expected default Swift version
// when swiftppc is invoked directly.
// It should /not/ follow the version specified when invoking lit, which means
// it can't use the %swiftppc_driver or %target-build-swift substitutions.
