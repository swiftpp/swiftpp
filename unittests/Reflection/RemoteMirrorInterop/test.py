...
swiftppcs = [os.path.join(arg, 'bin', 'swiftppc') for arg in absoluteArgs]
...
for i, (swiftppc, swiftlib) in enumerate(zip(swiftppcs, swiftlibs)):
    subprocess.check_call(
        ['xcrun', swiftppc, '-emit-library', 'test.swift',
...
for i in range(len(swiftppcs) + 1):
